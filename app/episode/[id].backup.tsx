...
import { getEpisodeDetail, getEpisodeComments, addRating, removeRating } from '../../services/traktApi';
...
export default function EpisodeDetailScreen() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch cast and backdrop from Show Cache
      const cacheKey = `@show_detail_${traktIdNum}`;
      const cacheData = await AsyncStorage.getItem(cacheKey);
      let fallbackBackdrop = null;
      if (cacheData) {
        const parsed = JSON.parse(cacheData);
        if (parsed.cast) {
          setCastData(parsed.cast.slice(0, 5));
        }
        fallbackBackdrop = parsed.backdrop;
      }
      ...
      setEpisodeData(detailRes);
      setCommentsData(commentsRes || []);
      setStillUrl(stillRes);
