 ...
 *
 * ⚠️ NEDEN YALNIZCA WEB: Google girişi GIS (Google Identity Services) web
 * script'iyle çalışıyor; native (iOS/Android) tarafı bilinçli olarak ertelendi
 * (gerçek Cloud Console client ID'leri + native SDK gerektiriyor).
 *
 ...
 * yani hiçbir yönlendirme gerekmez, tek bir ağ isteğiyle bağlanır.
 */
export function GoogleLinkSection() {
  ...
  // 🔴 Kapı DIŞ bileşende, `useGoogleSignIn` çağrılmadan ÖNCE. İç bileşene
  // ayrılmasının sebebi: hook'lar koşullu çağrılamaz, ama hook'u koşulsuz
  // çağırıp sonra `null` dönmek GIS'i görünmeyen kullanıcılar için de
  ...
  if (!isWeb || !traktKullanicisi || !accessToken) return null;

  return <GoogleLinkSectionInner traktAccessToken={accessToken} />;
}
