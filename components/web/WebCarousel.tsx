...
function WebCarouselInner({ title, data, renderItem, onViewAll }: WebCarouselProps) {
  ...
  const handleScroll = useCallback((event: any) => {
    ...
    // Aynı boolean değeri tekrar set edilirse React render'ı zaten atlar (bail-out).
...
// onViewAll bilinçli olarak karşılaştırma DIŞI: parent'lar onu genelde inline arrow
// olarak verir (her render'da yeni referans) ama closure'ın yakaladığı title/data zaten
// burada karşılaştırılıyor — onlar aynıysa onViewAll davranışı da aynıdır.
const WebCarousel = memo(WebCarouselInner, (prev, next) =>
  prev.title === next.title &&
  prev.data === next.data &&
  prev.renderItem === next.renderItem &&
  prev.viewAllText === next.viewAllText &&
